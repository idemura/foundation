...
typedef void (*testing_func_t)(struct testing_ctx *ctx);

#define TEST(FUNC) void FUNC(struct testing_ctx *testing_ctx)

#define TEST_REGISTER(FUNC) testing_register(#FUNC, &FUNC)

#define TEST_FAIL() do{ testing_fail(testing_ctx, __FILE__, __LINE__); return; }while(0)

#define EXPECT_EQL(E, A) \
    do { \
        if (!testing_expect_eql(testing_ctx, __FILE__, __LINE__, E, A)) return; \
    } while(0)

#define EXPECT_EQS(E, A) \
    do { \
        if (!testing_expect_eqs(testing_ctx, __FILE__, __LINE__, E, A)) return; \
    } while(0)

#define EXPECT_TRUE(A) \
    do { \
        if (!testing_expect_eqb(testing_ctx, __FILE__, __LINE__, true, A)) return; \
    } while(0)

#define EXPECT_FALSE(A) \
    do { \
        if (!testing_expect_eqb(testing_ctx, __FILE__, __LINE__, false, A)) return; \
    } while(0)
